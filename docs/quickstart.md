...
- **並行ジョブ複製 (`p` / `P` キー)**
  既存のログを選択し、`p` で複製先ジョブを指定して複製、あるいは `P` (Shift+p) で `[Unassigned]` として即時に同じ時間帯の重複（並行）ログを複製します。同じ時間帯に複数のログがある場合、エクスポート時にその重複時間は自動的に等分（按分）されて集計されます。
- **その他**: `x` で現在の作業の終了、`D` (Shift+d) で選択中のログの削除を行えます。

## 4. 現在のステータスの確認（CLI）

タイマーをバックグラウンドで実行している場合や、別のターミナルから現在の状態を確認したい場合は、`wtl status` コマンドを使用します。

```bash
# 現在のステータス（プロジェクト名、ジョブ名、経過時間など）を1回表示
wtl status

# ステータスを JSON 形式で出力（スクリプトや他のツールとの連携用）
wtl status --json

# 動的にステータスを監視（1秒ごとに自動更新）
wtl status --watch
```
